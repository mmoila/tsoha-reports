...
def get_all():
    sql = "SELECT A.id, A.date, A.title, A.description, C.username, B.ICAO_ident \
...
def send(date, title, description, id, user_id):
    sql = "INSERT INTO reports (date, title, description, location_id, user_id) VALUES \
        (:date, :title, :description, :location, :user_id)"
    db.session.execute(
